  ...
  return (
    <section className="py-20 md:py-32 overflow-hidden" id="safety" style={{ background: '#F5F6F8' }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        {/* Pillars — 2×2 grid with dividers */}
        <div className="grid sm:grid-cols-2 border-t border-l" style={{ borderColor: '#E5E8ED' }}>
          {pillars.map((p, i) => (
            <motion.div
              key={i}
              ...
              className="p-8 md:p-10 border-b border-r bg-white transition-shadow duration-300 group hover:shadow-md"
            ...
            >
              ...
              <h3 className="font-bold mb-3" style={{ color: '#0F1117' }}>{p.title}</h3>
        ...
        <motion.div
          ...
          className="mt-0 grid grid-cols-1 md:grid-cols-3 bg-white"
          style={{ border: '1px solid #E5E8ED', borderTop: 'none' }}
        >
          {[
            {
              icon: <svg viewBox="0 0 24 24" fill="none" stroke="#00D49A" strokeWidth="2" className="w-4 h-4 flex-shrink-0"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" /></svg>,
              text: 'All packages insured up to KSh 500,000 per delivery',
            },
            {
              icon: <svg viewBox="0 0 24 24" fill="none" stroke="#0EA5E9" strokeWidth="2" className="w-4 h-4 flex-shrink-0"><rect x="3" y="11" width="18" height="11" rx="2" /><path d="M7 11V7a5 5 0 0110 0v4" /></svg>,
              text: 'ISO 27001 certified data security infrastructure',
            },
            {
              icon: <svg viewBox="0 0 24 24" fill="none" stroke="#FF3D2E" strokeWidth="2" className="w-4 h-4 flex-shrink-0"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2" /><circle cx="9" cy="7" r="4" /><path d="M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75" /></svg>,
              text: '3,000+ background-checked riders across 8 cities',
            },
          ].map((item, i) => (
            <div key={i} className="flex items-center gap-3 px-6 py-5" style={{ borderRight: i < 2 ? '1px solid #E5E8ED' : undefined }}>
              {item.icon}
