...
function StarIcon() {
  return (
    <svg viewBox="0 0 20 20" fill="#FBC02D" className="w-4 h-4">
...
function TestimonialCard({ t }: { t: typeof testimonials[0] }) {
  return (
    <div className="w-[300px] sm:w-[340px] md:w-[360px] flex-shrink-0 glass rounded-3xl p-5 sm:p-7 border border-white/8 hover:border-white/15 transition-all duration-300 mx-3">
      <div className="flex gap-0.5 mb-5">
        {[...Array(t.rating)].map((_, j) => <StarIcon key={j} />)}
      </div>
      <p className="text-slate-200 leading-relaxed mb-6 text-[15px]">&ldquo;{t.quote}&rdquo;</p>
      <div className="flex items-center gap-3">
        <div className="w-11 h-11 rounded-full flex items-center justify-center text-xs font-black flex-shrink-0"
          style={{ background: `${t.color}25`, border: `1.5px solid ${t.color}40`, color: t.color }}>
          {t.initials}
        </div>
        <div>
...
export default function Testimonials() {
  ...
  return (
    <section className="relative py-20 md:py-32 overflow-hidden bg-bg-dark">
      <div className="absolute top-0 left-0 right-0 h-32 bg-gradient-to-b from-bg-deep to-transparent pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 mb-16">
        ...
        >
          <div className="inline-flex items-center gap-2 glass border border-primary/25 rounded-full px-4 py-2 mb-6">
            ...
            <span className="text-xs font-semibold text-slate-300 tracking-wide">50,000+ satisfied customers across Kenya</span>
          </div>
          <h2 className="text-4xl sm:text-5xl md:text-6xl font-black tracking-tight mb-6">
            Real people.<br /><span className="gradient-text">Real results.</span>
          </h2>
          <p className="text-slate-400 text-lg max-w-xl mx-auto">
            From solopreneurs to enterprise clients, see what DoorDash has done for people who trust us every day.
          </p>
        </motion.div>
      </div>

      {/* Auto-scrolling row 1 (left → right) */}
      <div className="ticker-container mb-5">
        <div className="ticker-track">
          {[...row1, ...row1].map((t, i) => <TestimonialCard key={i} t={t} />)}
        </div>
      </div>

      {/* Auto-scrolling row 2 (right → left) */}
